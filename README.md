**VeriTrail — The Verifiable Logistics Chain
About the Project**

VeriTrail is a web-based logistics and asset-transfer management system. The goal is to make internal transfers more organized, easier to track, and more accountable from the time a request is created until the item is received.

What We Worked On
Mainly worked on the system concept, frontend prototype, and overall user flow of VeriTrail.
The project is not yet a complete working system.
What we have now is mainly the frontend prototype, which demonstrates how VeriTrail is supposed to look and how the main user interactions and workflows are expected to work.

There is still a lot left to do, especially:
_Backend development
Database integration
Real authentication and authorization
API integration
Actual request processing
Persistent chain-of-custody records
Notifications
Incident and dispute management
Real GPS tracking
External courier integration
Complete role-based workflows
Testing and frontend-backend integration_

I might have overlooked some features that are still not completed, alas, the files are here to be checked.
So, at this stage, the frontend serves as the foundation and visual prototype of the system. The backend and database will still need to be developed and connected to make VeriTrail a fully functional application.

**Project Status**
Internship Deliverable:_ Frontend prototype and system foundation_
Current Stage: _Frontend prototype / pre-backend integration_
Backend:_ Not yet implemented_
Database:_ Not yet integrated_

This project can be continued by connecting the existing frontend to the planned backend, database, authentication, and operational workflows.
